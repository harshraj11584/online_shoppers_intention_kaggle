...
model = KNeighborsClassifier(n_neighbors=100,weights='uniform',p=1,n_jobs=-1)
model.fit(x_train, y_train)
yh_val = model.predict_proba(x_val)[:,1]

print(yh_val.shape)
print("AUC of KNeighborsClassifier: ",roc_auc_score(y_val,yh_val))

yh_test = model.predict_proba(x_test)[:,1]
np.save('m4_pred',yh_test)
print("yh_test.shape=",yh_test.shape)
np.save('m4_val',yh_val)
print("yh_val.shape=",yh_val.shape)
yh_train = model.predict_proba(x_train)[:,1]
np.save('m4_train',yh_train)
