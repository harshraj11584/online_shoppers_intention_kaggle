...
print("Creating F1: distance to nearest neighbours of each class")
...
for i in range(x_train.shape[0]):
	f1_train[i] = np.sum(np.sort(d_train_1[i])[-1:])
	f0_train[i] = np.sum(np.sort(d_train_0[i])[-1:])
...
for i in range(x_val.shape[0]):
	f1_val[i] = np.sum(np.sort(d_val_1[i])[-1:])
	f0_val[i] = np.sum(np.sort(d_val_0[i])[-1:])
...
for i in range(x_test.shape[0]):
	f1_test[i] = np.sum(np.sort(d_test_1[i])[-1:])
	f0_test[i] = np.sum(np.sort(d_test_0[i])[-1:])


np.save('F1_0_train.npy',f0_train)
np.save('F1_1_train.npy',f1_train)
np.save('F1_0_val.npy',f0_val)
np.save('F1_1_val.npy',f1_val)
np.save('F1_0_test.npy',f0_test)
np.save('F1_1_test.npy',f1_test)
